...
// Qt 5.12 中 javaScriptConsoleMessage 是 protected 虚函数（Qt 5.15+ 才改为信号），
// 因此通过 QWebEnginePage 子类重写它，把页面 JS 的 console 日志转发到 Qt 输出。
...
void CallWidget::setNetworkManager(NetworkManager* manager) {
    ...
    connect(networkManager, &NetworkManager::callSignalSendResult, this,
        ...
        if (code != 0 && callController->state() != VideoCallController::Idle) {
            ...
            const QString hint = signalType == QStringLiteral("call_request")
                ? QStringLiteral("对方不在线或无法接通，请稍后再试")
                : QStringLiteral("信令发送失败：%1").arg(message);
