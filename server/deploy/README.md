...
cd ~/xiaofu_vcall
cmake -S server -B server/build-linux-debug -DCMAKE_BUILD_TYPE=Debug
cmake --build server/build-linux-debug --target xiaofu-server -j"$(nproc)"

./server/build-linux-debug/xiaofu-server \
...
```bash
sudo bash server/deploy/install_ubuntu.sh
```
