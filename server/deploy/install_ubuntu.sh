...
if [[ ${EUID} -ne 0 ]]; then
    echo "请使用 sudo 运行：sudo bash server/deploy/install_ubuntu.sh"
    exit 1
fi

project_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
build_dir="${project_root}/server/build-linux-release"

apt-get update
apt-get install -y build-essential cmake libsqlite3-dev libssl-dev

cmake -S "${project_root}/server" -B "${build_dir}" -DCMAKE_BUILD_TYPE=Release
...
install -m 0644 "${project_root}/server/deploy/xiaofu-server.service" \
