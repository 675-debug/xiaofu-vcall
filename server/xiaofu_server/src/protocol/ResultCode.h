...
enum class ResultCode : int {
    ...
    UserOffline = 8,           // 用户离线
    UserBusy = 9,              // 用户正忙（通话中）
    CallNotFound = 10,         // 通话不存在
    CallAlreadyEnded = 11,     // 通话已结束
    InvalidCallState = 12,     // 当前通话状态不允许此操作
    NotCallParticipant = 13,   // 不是通话参与者
    CallTimeout = 14,          // 呼叫超时
    DuplicateLogin = 15,       // 重复登录（被踢下线）
    AccountAlreadyLoggedIn = 16,  // 该账户已在其他设备登录，拒绝本次登录
};
