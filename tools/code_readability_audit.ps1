...
$forbiddenTokens = @{
    'server/src/main.cpp' = @('conns', 'makeResp')
    'server/src/db/DbManager.cpp' = @('int rc', 'char* err', 'sqlite3_stmt* stmt', 'const unsigned char* col')
    'server/src/db/PasswordHasher.cpp' = @('hProv', 'hHash', 'std::stringstream ss')
    'server/src/net/Connection.h' = @('messageCb', 'closeCb', 'inBuf')
    'server/src/net/EventLoopWin.cpp' = @('timerCb', 'pfds', 'std::vector<int> ids')
    'server/src/net/TcpServer.h' = @('listenSock', 'acceptCb')
    'server/src/handler/JoinHandler.h' = @('fdToUser', 'kickCb')
...
$requiredComments = @{
    'server/src/db/PasswordHasher.cpp' = '密码加密函数'
    'server/src/net/Connection.cpp' = '解析长度头并拆分完整消息'
    'server/src/main.cpp' = '服务端启动流程'
}
...
$serverMain = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'server/src/main.cpp')
...
if ($serverMain -notmatch $deferredDeletePattern) {
    $failures.Add('server/src/main.cpp: closed connection must be deleted after onReadable returns')
}
