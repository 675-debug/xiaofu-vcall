...
# ====== 离线 ASR（已禁用：2G 内存跑不动离线大模型，实时字幕走 online 流式）======
# model_asr = AutoModel(
#     model="paraformer-zh",
...
async def async_asr(websocket, audio_in: bytes):
    ...
    if model_asr is None:
        # 离线 ASR 已禁用（内存瘦身），防御性返回空结果
        await websocket.send(json.dumps({
            "mode": mode, "spk_name": "unknown", "spk_score": 0.0,
            "text": "", "wav_name": websocket.wav_name, "is_final": True,
        }, ensure_ascii=False))
        return
